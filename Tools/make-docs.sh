...
# A simple script to generate a docarchive file for DSFAppearanceManager
...
xcodebuild docbuild -scheme DSFAppearanceManager -derivedDataPath "${temporary_folder}" -destination "platform=macOS" >/dev/null

echo "Copying docarchive into position..."
# Copy the resulting docarchive file up to the package folder
cp -R "${temporary_folder}/Build/Products/Debug/DSFAppearanceManager.doccarchive" .
